    ...
    export default {
      ...
      methods: {
        async initialize () {
          this.studentItem = []
          try {
            var data = await this.axios.get('http://localhost:8000/student')
        ...
        async deleteItemConfirm () {
          try {
            var response = await this.axios.delete('http://localhost:8000/student/' + this.idforDelete)
            this.initialize()
          } catch (error) {
        ...
        async save (action) {
          ...
          if (action === 'เพิ่มข้อมูล') {
            try {
              var dataResponse = await this.axios.post('http://localhost:8000/student', data)
            ...
            try {
              var dataResponse = await this.axios.put('http://localhost:8000/student/' + this.idstudent, data)
              console.log('dataResponse ====>', dataResponse)
              this.close()
              this.initialize()
            } catch (error) {
